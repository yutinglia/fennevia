...
| Visible tabs, one-line navigation/native-handoff toolbar, compact address/status launcher, centered address popup, right bookmarks panel, and bottom download-status panel | This project |
...
| --- | --- |
| `#navigator-toolbox` | Geometry collapses reversibly at active rest while it retains notification, titlebar, popup-anchor, customization, and mode ownership |
...
| `document.createElementNS("http://www.w3.org/1999/xhtml", ...)` and project-authored SVG in browser chrome | Current Firefox browser code, including [`browser-addons.js`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/base/content/browser-addons.js), explicitly creates HTML namespace nodes in chrome documents. SVG descendants retain the standard SVG namespace. | Every project host and structural frontend node uses the fixed XHTML namespace. The only allowed alternate namespace is the project-authored subtree rooted at `svg[data-fennevia-icon]`; all other mixed project namespaces fail health. |
...
| `TabAttrModified.detail.changed` values `soundplaying`, `muted`, `activemedia-blocked`, `usercontextid`, `attention`, `pictureinpicture` | The same `_tabAttrModified` path in [`tabbrowser.js`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/components/tabbrowser/content/tabbrowser.js) includes these names. [`tab.js`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/components/tabbrowser/content/tab.js) exposes the matching getters and `toggleMuteAudio()`. | Issue #60 reconciles these in addition to the #10 `label`/`image`/`busy`/`selected` set. Unrelated attributes remain ignored. Audio is a closed optional enum; attention and picture-in-picture are omitted unless true. `usercontextid` never leaves the bridge. |
| `gBrowser.moveTabTo(tab, { tabIndex, isUserTriggered })` | Current [`tabbrowser.js`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/components/tabbrowser/content/tabbrowser.js) clamps pinned and unpinned partitions for user moves. | Required. `move(id, index)` passes a fresh options record with `isUserTriggered: true`. Invalid indexes fail before the native call. |
| `#tabContextMenu` `openPopup` / `moveTo` / `popupshown` / `popuphidden` | [`main-popupset.js`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/base/content/main-popupset.js) runs `TabContextMenu.updateContextMenu` on `popupshowing`. That helper uses `triggerNode.tab`, `triggerNode.closest("tab")`, or `selectedTab`. | Required. The bridge opens the menu with the native `<tab>` as trigger, then `moveTo(screenX, screenY)`. `popupshown`/`popuphidden` publish a boolean hold event consumed by existing `setPopupHeld("left")`. Disposal calls `hidePopup` when present. Native menu/tab objects stay inside `src/firefox/`. |
| `ContextualIdentityService.getPublicIdentityFromId` | [`ContextualIdentityService.sys.mjs`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/toolkit/components/contextualidentity/ContextualIdentityService.sys.mjs) publishes closed color names and a public `name`. Aliases include `turquoise` → `cyan` and `toolbar` → `gray`. | Optional. `WindowShell.sys.mjs` may pass `ChromeUtils.importESModule` as `moduleLoader`. Load failure, private windows, missing identity, or unknown colors omit `container` and do not fail health. Labels are bounded text; color names are a closed enum. |
...
| `#downloads-button`, `DownloadsPanel.initialize()`, and `#downloadsPanel` | [`indicator.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/components/downloads/content/indicator.js) `DownloadsButton.getAnchor()` returns null when the parent toolbar fails `isElementVisible`. [`downloads.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/components/downloads/content/downloads.js) `showPanel()` then refuses to open. `initialize()` loads data without opening. | Required. The top-row action initializes the owner and `openPopup`s `#downloadsPanel` on the project host. Native chrome is not revealed. Filenames, actions, and safety state stay inside Firefox. |
| `#unified-extensions-button`, `gUnifiedExtensions.togglePanel()`, and `#unified-extensions-panel` | [`browser-unified-extensions.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/base/content/browser-unified-extensions.js) owns current extension identity, permissions, actions, and panel behavior. | The controller toggles the current owner without focusing the native button, then `moveToAnchor`/`openPopup` on the project host. It exports no extension ID/name/icon/widget/command. |
...
| `gCustomizeMode.enter()` and root `customizing` lifecycle | [`browser-customization.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/base/content/browser-customization.js) fronts current [`CustomizeMode.sys.mjs`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/components/customizableui/CustomizeMode.sys.mjs). | The Customize action delegates to Firefox. ADR-032 suspension exposes complete native UI throughout customization and revalidates exact owners afterward; Fennevia implements no customization model. |
...
The browser-tools public snapshot contains only nine fixed booleans and its
command accepts only nine fixed action strings plus an optional project-owned
host for the six popup actions. Popup position is `after_end` when the host is
...
| `-moz-window-dragging: drag` and `no-drag` | Firefox 153 [`toolkit/content/xul.css`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/toolkit/content/xul.css) marks eligible chrome toolbars as drag regions and excludes controls; [`browser-shared.css`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/themes/shared/browser-shared.css) applies the same browser-titlebar behavior. | Project-owned edge panels are drag regions; links, buttons, fields, editable nodes, and focusable controls are explicit no-drag descendants. Fennevia does not synthesize pointer movement. ADR-038 window buttons call Firefox window commands without clicking native caption nodes. |
...
| Project XHTML `#fennevia-native-ui-style` and CSSOM `cssRules` | A chrome-document XHTML style has synchronous parsed-rule access. | Health requires exact identity, frame parent, source text, and seven parsed rules. Missing/replaced style is `FENNEVIA_NATIVE_UI_STYLE_INVALID`; a non-seven rule parse is `FENNEVIA_NATIVE_UI_STYLE_PARTIAL`. Suspension is set before lifecycle fallback. |
...
The controller exposes only fixed capability records and synchronous boolean
`revealForUrlbar()` and `revealForToolbar()` actions to the Firefox boundary.
...
| Top browser-tool handoffs | Pinned Trust/identity/protections/permission/Downloads/Unified Extensions/PanelUI/settings/customization owners under **Single-line toolbar and native browser-tool handoffs** | `src/firefox/browser-tools.ts`; 21 required capabilities, nine fixed booleans/actions, optional project host for popup actions, privacy-safe popup hold, owner re-resolution, and deterministic window-reference disposal | Required; any owner/host/action drift fails the window open while Firefox's complete original toolbar and panels remain authoritative | adapter/controller/build/static tests plus pending real popup placement; first signal `FENNEVIA_FIREFOX_BROWSER_TOOLS_*` |
