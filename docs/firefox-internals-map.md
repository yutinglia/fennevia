...
`docs/research/firefox-153-single-line-toolbar-handoffs.md`; ADR-037 records the
fixed-action architecture.

| Dependency | Firefox 153 source-backed behavior | Project owner and failure behavior |
| --- | --- | --- |
| `#trust-icon-container`, `#identity-icon-box`, and `#tracking-protection-icon-container` | [`navigator-toolbox.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/base/content/navigator-toolbox.js) routes the current Trust anchor to `gTrustPanelHandler`, the identity anchor to Trust while `trustPanel.featureGate` is enabled and otherwise `gIdentityHandler`, and the legacy protection anchor to `gProtectionsHandler`. | `src/firefox/browser-tools.ts` validates all three owners. Site-information and protections use the visible Trust owner when available and their respective legacy owner otherwise. It reveals the original toolbar, focuses and clicks only the re-resolved original owner, and exports no handler, certificate, tracker, exception, URL, or panel data. |
...
| `#identity-permission-box` and `gPermissionPanel.handleIdentityButtonEvent()` | [`browser-sitePermissionPanel.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/base/content/browser-sitePermissionPanel.js) initializes, populates, anchors, and owns current site-permission controls. | The fixed site-permissions action reveals and activates the original owner. Permission IDs, states, scopes, origins, sharing data, labels, and actions never cross the browser-tools bridge. Missing owner fails health; collapsed-anchor popup placement remains in the manual matrix. |
| `#downloads-button` and `DownloadsIndicatorView.onCommand()` | `navigator-toolbox.js` delegates current keyboard activation to the Downloads indicator; [`indicator.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/components/downloads/content/indicator.js) owns the complete native panel. | The top-row action activates the original button after reveal, so Firefox displays filenames, actions, history, and safety state. The bottom edge remains the separate anonymous aggregate status from `src/firefox/downloads.ts`; no new download data crosses. |
| `#unified-extensions-button` and `gUnifiedExtensions.togglePanel()` | [`browser-unified-extensions.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/base/content/browser-unified-extensions.js) owns current extension identity, permissions, actions, and panel behavior. | The controller focuses the original anchor and invokes its current owner. It exports no extension ID/name/icon/widget/command. Arbitrary pinned widgets remain available through the complete original-toolbar handoff. |
| `#PanelUI-menu-button` and `PanelUI.show()` | [`panelUI.js`](https://github.com/mozilla-firefox/firefox/blob/c178247e1dfea52241a6b18b18cf3a00f8da935c/browser/components/customizableui/content/panelUI.js) owns the application menu, subviews, and popup lifecycle. | The fixed action reveals/focuses the original anchor and calls the current per-window owner. Menu contents and commands remain native and absent from project state. |
...
command accepts only nine fixed action strings. It has no subscription because
these required owners are stable for the supported window; every action
re-resolves its owner, and disposal drops the window reference. Required owner,
reveal, action, and malformed-result failures retain fixed privacy-safe
diagnostics and follow ADR-021 fail-open.
...
| Top browser-tool handoffs | Pinned Trust/identity/protections/permission/Downloads/Unified Extensions/PanelUI/settings/customization owners under **Single-line toolbar and native browser-tool handoffs** | `src/firefox/browser-tools.ts`; fixed per-window booleans/actions, owner re-resolution, and deterministic window-reference disposal | Required; any owner/reveal/action drift fails the window open while Firefox's complete original toolbar and panels remain authoritative | adapter/controller/build/static tests plus pending real popup/toolbar/caption matrix; first signal `FENNEVIA_FIREFOX_BROWSER_TOOLS_*` |
