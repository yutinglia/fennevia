...
| OS titlebar and Firefox window controls | Platform integration; ADR-037 styles only retained native nodes in place |
...
| Three `.titlebar-buttonbox-container` groups, their `.titlebar-buttonbox`, and min/max/restore/close children | [`titlebar-items.inc.xhtml`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/base/content/titlebar-items.inc.xhtml) places current platform controls in menubar, tab-toolbar, and navbar contexts selected by Firefox mode/style. Firefox's [`browser-shared.css`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_153_0_4_RELEASE/browser/themes/shared/browser-shared.css#L327-L416) may hide both the container and inner box according to `customtitlebar`, menubar, and tab orientation; it also places the toolbox below the tabbox stacking level by default. | Creation requires exactly one group in each owner and all four button classes. Resting active CSS explicitly displays the selected native container and inner box, raises the zero-height toolbox stacking context, and keeps the group fixed. While the project top surface is visible, `:has(#fennevia-shell-frame-host[data-fennevia-top-visible])` moves it below the top panel. Fennevia never reparents, replaces, or invokes the native buttons. Missing/replaced controls prevent health. |
...
| Project XHTML `#fennevia-native-ui-style` and CSSOM `cssRules` | A chrome-document XHTML style has synchronous parsed-rule access. | Health requires exact identity, frame parent, source text, and thirteen parsed rules. Missing/replaced style is `FENNEVIA_NATIVE_UI_STYLE_INVALID`; a non-thirteen rule parse is `FENNEVIA_NATIVE_UI_STYLE_PARTIAL`. Suspension is set before lifecycle fallback. |
