...
| Four ordered XHTML edge hosts and exact mount targets | Current browser chrome supports explicit XHTML creation in the mixed-namespace document; the frame remains a sibling of Firefox-owned content infrastructure | The frame owns top, left, right, and bottom hosts in that order. Each host owns one target and Svelte root. CSS is a separate frame child. Partial attach, mount, health, or disposal failure is all-or-nothing and leaves native UI visible. |
...
The public snapshot contains only five primitive fields. The title and display
URI are page-derived but bounded, text-only, per-window, and prohibited from
logs/errors/datasets/persistence/transmission. The top Svelte surface receives
only the application adapter and uses ADR-026's existing host/controller; no
native handle, command element, event, observer, or Firefox implementation
module crosses into UI code. Revalidate every row on the next supported
Firefox stable.
