...
| `ChromeUtils.importESModule()` | Current Firefox source has many privileged `chrome://` ESM callers; the real spike imported the newly registered entry immediately | AutoConfig imports one fixed entry URI. The entry imports only fixed Logger, WindowManager, WindowShell, Runtime, and PrivateBrowsingUtils URIs and validates the frozen result contract. Syntax, missing-module, and import failures retain phase, redacted full stack, version, and build ID; the #5 missing-module probe retained native UI. |
