...
- public package `0.11.0-beta.1` prerelease;
...
evidence. The owner-observed Firefox 154 AMO path currently uses the accepted
...
- drag ghost/insertion preview carries only the opaque tab ID and clears its
  preview plus shared pointer hold on every terminal path.
