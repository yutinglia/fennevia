...
The current package is `0.13.0-beta.1` and contains 16 source-installed files:
...
`.mjs`, `.js`, `.css`, and notice files to LF so the committed SHA-256 values remain
...
pwsh -NoProfile -File .\scripts\verify-release.ps1 `
  -PackageRoot .\.release\fennevia-0.13.0-beta.1
```
