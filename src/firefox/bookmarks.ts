...
const PLACES_UTILS_URI = "resource://gre/modules/PlacesUtils.sys.mjs";
const PLACES_UI_UTILS_URI =
  "moz-src:///browser/components/places/PlacesUIUtils.sys.mjs";
const BOOKMARK_EVENT_TYPES = Object.freeze([
  "bookmark-added",
  "bookmark-removed",
  "bookmark-moved",
...
const MAXIMUM_EVENT_PARENTS = 16;
const MAXIMUM_EVENT_BATCH = 128;
const MAXIMUM_QUERY_OFFSET = 1_000_000;
const GUID_PATTERN = /^[A-Za-z0-9_-]{12}$/u;
...
type NativeBookmarkHandle = Readonly<{ guid: string }>;
type NativeBookmarkRecord = NativeRecord & {
  childCount?: unknown;
  guid: string;
  index: number;
  parentGuid: string;
...
type NativeBookmarks = NativeRecord & {
...
type NativePlacesObservers = NativeRecord & {
  addListener: (
...
type NativePlacesUtils = NativeRecord & {
  bookmarks: NativeBookmarks;
  observers: NativePlacesObservers;
};
type NativePlacesUIUtils = NativeRecord & {
  openNodeIn: (
    node: unknown,
    where: "current" | "tab",
    view: NativeRecord,
...
type BookmarksCapabilityEvaluation = Readonly<{
  cause?: unknown;
  snapshot: FirefoxCapabilitySnapshot;
}>;

type BookmarksCapabilitySpecification = Readonly<{
...
const getUrlProtocol = (value: unknown): string | null => {
