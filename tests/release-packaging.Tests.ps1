...
try {
    ...
    Assert-Equal -Actual $first.Version -Expected "0.18.0-beta.1" -Message "The canonical package version must drive release staging."
    Assert-Equal -Actual $first.Tag -Expected "v0.18.0-beta.1" -Message "The exact tag must be derived from the package version."
    Assert-True -Condition $first.Prerelease -Message "The public package must remain marked as a prerelease."
    Assert-Equal -Actual $first.ArchiveName -Expected "fennevia-0.18.0-beta.1-windows.zip" -Message "The archive name must be deterministic and versioned."
    ...
    Assert-Equal -Actual ([string] $validation.Manifest.source.archive) -Expected "https://github.com/yutinglia/fennevia/archive/refs/tags/v0.18.0-beta.1.zip" -Message "The release manifest must identify corresponding preferred source."
    ...
    try {
        ...
        Assert-True -Condition (@($entryNames | Where-Object { -not $_.StartsWith("fennevia-0.18.0-beta.1/", [StringComparison]::Ordinal) }).Count -eq 0) -Message "Every ZIP entry must remain under one versioned top-level directory."
    ...
    $extractedRoot = Join-Path $extractRoot "fennevia-0.18.0-beta.1"
