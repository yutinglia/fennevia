...
Assert-True -Condition ($workflow.Contains('for ($attempt = 1; $attempt -le 10; $attempt++)')) -Message "Private-draft discovery must use a statically bounded retry after creation."
Assert-True -Condition ($workflow.Contains('Start-Sleep -Seconds 2')) -Message "Private-draft discovery must allow GitHub's release list a short eventual-consistency interval."
Assert-True -Condition ($workflow.Contains('$matchingReleases.Count -gt 1')) -Message "Private-draft discovery must fail immediately if the tag is no longer unique."
Assert-True -Condition ($workflow.Contains('$candidateAssetCount -gt $expectedAssets.Count')) -Message "Private-draft discovery must never wait through an excess-asset state."
