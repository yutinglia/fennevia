...
Fennevia 已經超越首個四邊介面 MVP。目前預發行版亦包括 Fennevia 自有的 widget 編輯器，可把 widget 即時拖放到四個邊緣；有限度的外觀與動效設定；預設跟隨 Firefox 設計 token；英文與繁體中文介面；啟動首幀隱藏原生工具列；以及 Windows 的 `FenneviaSetup.exe` 安裝精靈。
...
把介面形容為「不可設定」已經不再準確。Fennevia 自訂模式可以把支援的 Firefox 工具列 widget 放到任何邊緣，亦可調整有限度、儲存在設定檔內的面板／視窗背景、文字、邊框、飽和度、陰影及動效數值。它刻意不是通用 CSS 編輯器、擴充平台或可無限制改變幾何結構的介面產生器。
